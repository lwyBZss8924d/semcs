...
impl Span {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.line_start == 0 {
            return Err(CcError::SpanValidation(
        ...
        if self.byte_start > self.byte_end {
            return Err(CcError::SpanValidation(format!(
...
pub enum SearchMode {
    ...
    Semantic,
    Hybrid,
    Ast,  // AST structural search using ast-grep
}
...
pub struct SearchOptions {
    ...
    // AST-specific options (for --ast mode)
    pub ast_pattern: Option<String>,      // AST pattern (overrides query if set)
    pub ast_lang: Option<String>,         // Force language for AST search
    pub ast_selector: Option<String>,     // AST kind selector
    pub ast_strictness: Option<String>,   // Matching strictness (cst/smart/ast/relaxed/signature)
}
...
/// Get default .csignore file content
pub fn get_default_csignore_content() -> &'static str {
    r#"# .csignore - Default patterns for cs semantic search
...
/// Read and parse .csignore file, returning patterns
pub fn read_csignore_patterns(repo_root: &Path) -> Result<Vec<String>> {
    let csignore_path = repo_root.join(".csignore");

    if !csignore_path.exists() {
        return Ok(Vec::new());
    }

    let content = std::fs::read_to_string(&csignore_path).map_err(CcError::Io)?;
...
/// Create .csignore file with default content if it doesn't exist
pub fn create_csignore_if_missing(repo_root: &Path) -> Result<bool> {
    let csignore_path = repo_root.join(".csignore");
    ...
    std::fs::write(&csignore_path, get_default_csignore_content()).map_err(CcError::Io)?;
...
pub fn build_exclude_patterns(
    ...
    // 1. Load .csignore patterns (highest priority among additional patterns)
    if use_csignore
        && let Some(root) = repo_root
        && let Ok(csignore_patterns) = read_csignore_patterns(root)
        && !csignore_patterns.is_empty()
    {
...
pub fn get_sidecar_path(repo_root: &Path, file_path: &Path) -> PathBuf {
    ...
    let mut sidecar = repo_root.join(".cs");
    sidecar.push(relative);
    let ext = relative
        .extension()
        .map(|e| format!("{}.cs", e.to_string_lossy()))
...
pub mod pdf {
    ...
    pub fn get_content_cache_path(repo_root: &Path, file_path: &Path) -> PathBuf {
        let relative = file_path.strip_prefix(repo_root).unwrap_or(file_path);
        let mut cache_path = repo_root.join(".cs").join("content");
    ...
    mod tests {
        ...
        fn test_get_content_cache_path() {
            ...
            assert_eq!(
                cache_path,
                PathBuf::from("/project/.cs/content/docs/manual.pdf.txt")
            );
        }
        ...
        fn test_get_content_cache_path_no_extension() {
            ...
            assert_eq!(
                cache_path,
                PathBuf::from("/project/.cs/content/docs/manual.txt")
            );
        }
...
mod tests {
    ...
    fn test_span_validation_invalid_byte_range() {
        ...
        if let Err(CcError::SpanValidation(msg)) = result {
    ...
    fn test_get_sidecar_path() {
        ...
        let expected = PathBuf::from("/home/user/project/.cs/src/main.rs.cs");
    ...
    fn test_get_sidecar_path_no_extension() {
        ...
        let sidecar = get_sidecar_path(&repo_root, &file_path);
        let expected = PathBuf::from("/project/.cs/README.cs");
    ...
    #[test]
    fn test_create_csignore_if_missing() {
        let temp_dir = TempDir::new().unwrap();
        let test_path = temp_dir.path();

        // First creation should succeed
        let created = create_csignore_if_missing(test_path).unwrap();
        assert!(created);

        // Check that file exists
        let csignore_path = test_path.join(".csignore");
        assert!(csignore_path.exists());

        // Check content contains expected patterns
        let content = fs::read_to_string(&csignore_path).unwrap();
        ...
        let created_again = create_csignore_if_missing(test_path).unwrap();
        assert!(!created_again);
    }

    #[test]
    fn test_read_csignore_patterns() {
        let temp_dir = TempDir::new().unwrap();
        let test_path = temp_dir.path();

        // Test with no .csignore file
        let patterns = read_csignore_patterns(test_path).unwrap();
        assert_eq!(patterns.len(), 0);

        // Create a .csignore file
        let csignore_path = test_path.join(".csignore");
        fs::write(
            &csignore_path,
            r#"# Comment line
...
"#,
    ...
    #[test]
    fn test_read_csignore_patterns_with_empty_lines() {
        let temp_dir = TempDir::new().unwrap();
        let test_path = temp_dir.path();
...
"#,
    ...
    #[test]
    fn test_get_default_csignore_content() {
