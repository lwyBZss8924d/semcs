...
fn test_basic_grep_functionality() {
    ...
    let output = Command::new(cc_binary())
...
fn test_index_command() {
    ...
    // Check that .cc directory was created
    assert!(temp_dir.path().join(".cc").exists());
}

fn read_manifest_updated(dir: &Path) -> u64 {
    let manifest_path = dir.join(".cc").join("manifest.json");
...
fn test_error_handling() {
    ...
    let _output = Command::new(cc_binary())
...
fn test_jsonl_vs_regular_output() {
    ...
    // Regular output
    let regular_output = Command::new(cc_binary())
        .args(["fn main", temp_dir.path().to_str().unwrap()])
        .output()
        .expect("Failed to run cc");

    // JSONL output
    let jsonl_output = Command::new(cc_binary())
        .args(["fn main", "--jsonl", temp_dir.path().to_str().unwrap()])
        .output()
        .expect("Failed to run cc");
