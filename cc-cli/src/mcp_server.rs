...
fn filter_valid_results(mut results: Vec<cc_core::SearchResult>) -> Vec<cc_core::SearchResult> {
...
fn resolve_exclude_patterns(
    ...
    if let Ok(ccignore_patterns) = cc_core::read_ccignore_patterns(base_path) {
    ...
    fn get_info(&self) -> InitializeResult {
        InitializeResult {
            ...
            server_info: Implementation {
                ...
                website_url: Some("https://github.com/BeaconBay/cc".to_string()),
                icons: None,
            },
...
impl CcMcpServer {
    ...
    fn create_tool_router() -> ToolRouter<Self> {
        ...
        router.add_route(Self::default_ccignore_route());
        router
    }

    fn default_ccignore_route() -> ToolRoute<Self> {
        ...
        let tool = Tool {
            name: "default_ccignore".into(),
            title: Some("Default .ccignore".into()),
            description: Some("Retrieve the default .ccignore content generated by cc".into()),
        ...
        ToolRoute::new_dyn(tool, |_context: ToolCallContext<'_, CcMcpServer>| {
            Box::pin(async move {
                ...
                let summary = "Default .ccignore patterns for cc".to_string();
    ...
    pub async fn handle_semantic_search(
        ...
        // Note: Embedders are created fresh for each request by cc-engine
        ...
        let search_results = match cc_engine::search_enhanced_with_indexing_progress(
            &options,
            None,
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                if message.contains("No embeddings found") {
                    ...
                    match cc_engine::search_enhanced_with_indexing_progress(
                        &reindex_options,
                        None,
                        None,
                        None,
    ...
    pub async fn handle_lexical_search(
        ...
        let started = Instant::now();
        let search_results =
            match cc_engine::search_enhanced_with_indexing_progress(&options, None, None, None)
                .await
            {
                Ok(results) => results,
    ...
    async fn handle_index_status(
        ...
        let index_path = path_buf.join(".cc");
        ...
        if index_exists {
            ...
            // Get detailed index statistics using cc_index with caching
            ...
            } else if let Ok(index_stats) = cc_index::get_index_stats(&path_buf) {
    ...
    async fn handle_reindex(
        ...
        let reindex_result = match cc_engine::search_enhanced_with_indexing_progress(
            &options,
