use anyhow::Result;
use cc_core::{CcError, IncludePattern, SearchMode, SearchOptions, SearchResult, Span};
...
pub type DetailedIndexingProgressCallback = Box<dyn Fn(cc_index::EmbeddingProgress) + Send + Sync>;
    ...
    if cc_core::pdf::is_pdf_file(file_path) {
        ...
        if !cache_path.exists() {
            return Err(anyhow::anyhow!(
                "PDF not preprocessed. Run 'cc --index' first."
            ));
        }
        Ok(cache_path)
    } else {
...
async fn extract_content_from_span(file_path: &Path, span: &cc_core::Span) -> Result<String> {
...
fn find_model_entry<'a>(
    registry: &'a cc_models::ModelRegistry,
    key: &str,
) -> Option<(String, &'a cc_models::ModelConfig)> {
...
pub(crate) fn resolve_model_from_root(
    ...
    use cc_models::ModelRegistry;

    let registry = ModelRegistry::default();
    let index_dir = index_root.join(".cc");
    let manifest_path = index_dir.join("manifest.json");

    if manifest_path.exists() {
        let data = std::fs::read(&manifest_path)?;
        let manifest: cc_index::IndexManifest = serde_json::from_slice(&data)?;
            ...
            if let Some(requested) = cli_model {
                ...
                if requested_config.name != existing_model {
                    ...
                    return Err(CcError::Embedding(format!(
                        "Index was built with embedding model '{}' (alias '{}'), but '--model {}' was requested. To switch models run `cc --clean .` then `cc --index --model {}`. To keep using this index rerun your command with '--model {}'.",
                        existing_model,
                        suggested_alias,
                        requested,
                        requested,
...
pub async fn search_enhanced(options: &SearchOptions) -> Result<cc_core::SearchResults> {
...
pub async fn search_enhanced_with_indexing_progress(
    ...
    if !options.path.exists() {
        return Err(cc_core::CcError::Search(format!(
            "Path does not exist: {}",
...
fn regex_search(options: &SearchOptions) -> Result<Vec<SearchResult>> {
    ...
    let files = if should_recurse {
        // Use cc_index's collect_files which respects gitignore
        let collected = cc_index::collect_files(
            &options.path,
            options.respect_gitignore,
            &options.exclude_patterns,
        )?;
    ...
    for (line_idx, line) in lines.iter().enumerate() {
        ...
        if regex.as_str().is_empty() {
            ...
            results.push(SearchResult {
                ...
                score: 1.0,
                preview,
                lang: cc_core::Language::from_path(file_path),
                symbol: None,
                chunk_hash: None,
                index_epoch: None,
            });
...
async fn lexical_search(options: &SearchOptions) -> Result<Vec<SearchResult>> {
    // Handle both files and directories and reuse nearest existing .cc index up the tree
    ...
    let index_dir = index_root.join(".cc");
    if !index_dir.exists() {
        return Err(CcError::Index("No index found. Run 'cc index' first.".to_string()).into());
    }
    ...
    for (_score, doc_address) in top_docs {
        ...
        raw_results.push((
            ...
            SearchResult {
                ...
                score: _score,
                preview,
                lang: cc_core::Language::from_path(&PathBuf::from(path_text)),
                symbol: None,
                chunk_hash: None,
                index_epoch: None,
            },
...
async fn build_tantivy_index(options: &SearchOptions) -> Result<Vec<SearchResult>> {
    ...
    let tantivy_index_path = index_root.join(".cc").join("tantivy_index");
    let mut schema_builder = Schema::builder();
    let content_field = schema_builder.add_text_field("content", TEXT | STORED);
    let path_field = schema_builder.add_text_field("path", TEXT | STORED);
    let _schema = schema_builder.build();
...
async fn ensure_index_updated_with_progress(
    ...
    progress_callback: Option<cc_index::ProgressCallback>,
    detailed_progress_callback: Option<cc_index::DetailedProgressCallback>,
    respect_gitignore: bool,
    exclude_patterns: &[String],
    model_override: Option<&str>,
) -> Result<()> {
    // Find index root for .cc directory location
    ...
    if force_reindex {
        let stats = cc_index::smart_update_index_with_detailed_progress(
            index_root,
            true,
            progress_callback,
...
fn extract_code_sections(file_path: &Path, content: &str) -> Option<Vec<(usize, usize, String)>> {
    let lang = cc_core::Language::from_path(file_path)?;

    // Parse the file with tree-sitter and extract function/class sections
    if let Ok(chunks) = cc_chunk::chunk_text(content, Some(lang)) {
        let sections: Vec<(usize, usize, String)> = chunks
            .into_iter()
            .filter(|chunk| {
                matches!(
                    chunk.chunk_type,
                    cc_chunk::ChunkType::Function
                        | cc_chunk::ChunkType::Class
                        | cc_chunk::ChunkType::Method
                )
            })
            .map(|chunk| {
                (
