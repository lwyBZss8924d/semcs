use anyhow::Result;
use cc_core::{CcError, SearchOptions, SearchResult};
use std::path::Path;
use walkdir::WalkDir;
...
pub async fn semantic_search_v3(options: &SearchOptions) -> Result<cc_core::SearchResults> {
...
pub async fn semantic_search_v3_with_progress(
    ...
    if !index_dir.exists() {
        return Err(CcError::Index(
            "Index creation failed. Please try running 'cc --index' explicitly.".to_string(),
        )
        .into());
    }
    ...
    let mut file_chunks: Vec<(std::path::PathBuf, cc_index::ChunkEntry)> = Vec::new();
    ...
    for entry in WalkDir::new(&index_dir) {
        ...
        if entry.file_type().is_file() {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("cc") {
                ...
                if let Ok(index_entry) = cc_index::load_index_entry(path) {
    ...
    if file_chunks.is_empty() {
        return Err(CcError::Index(
            "No embeddings found. Run 'cc --index' first with embeddings.".to_string(),
        )
        .into());
    }
    ...
    let mut embedder = cc_embed::create_embedder(Some(resolved_model.canonical_name.as_str()))?;
    let query_embeddings = embedder.embed(std::slice::from_ref(&options.query))?;

    if query_embeddings.is_empty() {
        return Ok(cc_core::SearchResults {
            matches: Vec::new(),
    ...
    let mut similarities: Vec<(f32, &std::path::PathBuf, &cc_index::ChunkEntry)> = Vec::new();
    ...
    if options.rerank && !results.is_empty() {
        ...
        match cc_embed::create_reranker(rerank_model_name) {
            Ok(mut reranker) => {
...
fn reconstruct_original_path(
    ...
    // Remove the index directory prefix and .cc extension
