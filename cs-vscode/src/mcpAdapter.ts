...
import { CcConfig, IndexStatus, SearchOptions, SearchResponse, SearchResult, IndexProgressUpdate } from './types';
...
export class CcMcpAdapter {
  private child: ChildProcessWithoutNullStreams | undefined;
  private readlineInterface: readline.Interface | undefined;
  private pendingRequests = new Map<number, PendingRequest>();
  private nextId = 1;
  private initializationPromise: Promise<void> | undefined;
  private outputChannel = vscode.window.createOutputChannel('cc MCP');
  ...
  constructor(private readonly config: CcConfig, private readonly indexRoot: string) {}
  ...
  async getDefaultCcignoreContent(_indexRoot: string): Promise<string> {
    await this.ensureServer();
    const response = await this.callTool('default_ccignore', {});
    const content = response?.ccignore ?? response?.content ?? response;
    if (typeof content === 'string') {
      return content;
    }
    throw new Error('MCP server did not return default .ccignore content');
  }
  ...
  private spawnServer(): void {
    ...
    const command = this.config.mcpCommand || this.config.cliPath || 'cc';
    ...
    this.child.on('error', (err) => {
      vscode.window.showErrorMessage(`cc MCP server failed: ${err.message}`);
      this.rejectAllPending(err);
    });

    this.child.on('exit', (code, signal) => {
      const message = code !== null ? `exit code ${code}` : `signal ${signal}`;
      this.outputChannel.appendLine(`[cc-mcp] server exited (${message})`);
    ...
    this.outputChannel.appendLine('[cc-mcp] server spawned');
  }
  ...
  private async initializeServer(): Promise<void> {
    try {
      ...
      this.outputChannel.appendLine('[cc-mcp] initialization complete');
    } catch (error) {
      this.outputChannel.appendLine(`[cc-mcp] initialization failed: ${(error as Error).message}`);
  ...
  private async sendNotification(method: string, params?: Record<string, unknown>): Promise<void> {
    ...
    try {
      ...
      this.outputChannel.appendLine(`[cc-mcp] failed to send notification ${method}: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  private handleLine(line: string): void {
    ...
    try {
      ...
      this.outputChannel.appendLine(`[cc-mcp] failed to parse JSON: ${trimmed}`);
      return;
    }
    ...
    switch (notification.method) {
      case '$/progress':
        if (notification.params?.message) {
          this.outputChannel.appendLine(`[cc-mcp] ${notification.params.message}`);
        }
        ...
        break;
      default:
        this.outputChannel.appendLine(`[cc-mcp] notification ${notification.method}`);
    }
  }
