...
impl AnnIndex for SimpleIndex {
        ...
        Self: Sized,
    {
        ...
        for (i, vector) in vectors.iter().enumerate() {
            if vector.len() != dim {
                bail!(
                    "Embedding size mismatch while building index: expected {dim} values but vector #{i} has {}. This usually means different embedding models were mixed. Clean the index (`cc --clean .`) and rebuild with a single model, or rerun your command using the same `--model` you originally indexed with.",
                    vector.len()
                );
            }
        }
    ...
    fn search(&self, query: &[f32], topk: usize) -> Result<Vec<(u32, f32)>> {
        if self.dim == 0 {
            bail!(
                "The ANN index is empty. Reindex the repository before running semantic search (`cc --index`)."
            );
        }

        if query.len() != self.dim {
            bail!(
                "Embedding size mismatch during search: this index stores vectors with {expected} values, but the query provided {actual}. This happens when different embedding models are mixed. Re-run the command with the original model or clean the index (`cc --clean .`) and rebuild with a single model.",
                expected = self.dim,
                actual = query.len()
            );
        }
    ...
    fn add(&mut self, id: u32, vector: &[f32]) -> Result<()> {
        ...
        if vector.len() != self.dim {
            bail!(
                "Embedding size mismatch while updating index: expected {} values but received {}. To switch models, clean the index (`cc --clean .`) and rebuild with the new model. Otherwise rerun your command using the original `--model`.",
                self.dim,
                vector.len()
            );
        }
