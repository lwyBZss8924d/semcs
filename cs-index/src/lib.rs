use anyhow::Result;
use cs_core::{FileMetadata, Language, Span, compute_file_hash, get_sidecar_path};
...
pub fn collect_files(
    ...
    if respect_gitignore {
        ...
        use cs_core::get_default_exclude_patterns;
...
pub async fn index_directory(
    ...
    let resolved_model = if compute_embeddings {
        ...
        let model_registry = cs_models::ModelRegistry::default();
    ...
    if compute_embeddings {
        ...
        let mut embedder = cs_embed::create_embedder(resolved_model.as_deref())?;
...
pub async fn index_file(file_path: &Path, compute_embeddings: bool) -> Result<()> {
    ...
    let index_dir = repo_root.join(".cs");
    fs::create_dir_all(&index_dir)?;
    ...
    let entry = if compute_embeddings {
        ...
        let mut embedder = cs_embed::create_embedder(model_name)?;
    ...
    let (resolved_model, _model_dimensions) = if compute_embeddings {
        ...
        let (final_model, final_dims) = if let Some(existing_model) = &manifest.embedding_model {
            ...
            if model.is_none() {
                ...
                let existing_dims = manifest.embedding_dimensions.unwrap_or(384);
                if model_dims != existing_dims {
                    return Err(anyhow::anyhow!(
                        "Model mismatch: Index was created with '{}' ({} dims), but you're trying to use '{}' ({} dims). \
                        Dimension mismatch prevents cross-model queries. Please run 'cc --clean .' to remove the old index, then 'cc --index --model {}' to rebuild with the new model.",
                        existing_model,
                        existing_dims,
                        selected_model,
                        model_dims,
                ...
                // Dimensions match - allow cross-model query
                // Keep the existing model for indexing (index is already complete)
                // The query will use the requested model via resolve_model_from_root()
                tracing::info!(
                    "Cross-model query detected: index uses '{}' ({} dims), query will use '{}' ({} dims)",
                    existing_model,
                    existing_dims,
                    selected_model,
                    model_dims
                );
                (existing_model.clone(), existing_dims)
            } else {
                // Model matches exactly, proceed
...
fn index_single_file(
    file_path: &Path,
    repo_root: &Path,
    embedder: Option<&mut Box<dyn cs_embed::Embedder>>,
) -> Result<IndexEntry> {
...
fn index_single_file_with_progress(
    ...
    let lang = if cs_core::pdf::is_pdf_file(file_path) {
        Some(Language::Pdf)
    } else {
        cs_core::Language::from_path(file_path)
    };

    let model_name = embedder.as_ref().map(|e| e.model_name());
    let chunks = cs_chunk::chunk_text_with_model(&content, lang, model_name)?;
        ...
        if let Some(ref callback) = detailed_progress {
            ...
            for (chunk_index, chunk) in chunks.into_iter().enumerate() {
                ...
                let chunk_type_str = match chunk.chunk_type {
                    cs_chunk::ChunkType::Function => Some("function".to_string()),
                    cs_chunk::ChunkType::Class => Some("class".to_string()),
                    cs_chunk::ChunkType::Method => Some("method".to_string()),
                    cs_chunk::ChunkType::Module => Some("module".to_string()),
                    cs_chunk::ChunkType::Text => None,
                };
...
fn find_repo_root(path: &Path) -> Result<PathBuf> {
    ...
    loop {
        if current.join(".cs").exists() || current.join(".git").exists() {
...
fn is_text_file(path: &Path) -> bool {
    ...
    if cs_core::pdf::is_pdf_file(path) {
        return true;
    }
...
mod tests {
    ...
    impl cs_embed::Embedder for EmptyResultsEmbedder {
    ...
    impl cs_embed::Embedder for MismatchedCountEmbedder {
        fn id(&self) -> &'static str {
    ...
    fn test_index_single_file_handles_empty_embedding_results() {
        ...
        let mut empty_embedder: Box<dyn cs_embed::Embedder> = Box::new(EmptyResultsEmbedder);
    ...
    fn test_index_single_file_handles_mismatched_embedding_count() {
        ...
        let mut mismatched_embedder: Box<dyn cs_embed::Embedder> =
    ...
    fn test_index_single_file_with_valid_embedder_still_works() {
        ...
        let dummy_embedder = cs_embed::DummyEmbedder::new();
        let mut boxed_embedder: Box<dyn cs_embed::Embedder> = Box::new(dummy_embedder);
    ...
    fn test_cleanup_index() {
        ...
        let index_dir = test_path.join(".cs");
    ...
    fn test_sidecar_to_original_path() {
        let temp_dir = TempDir::new().unwrap();
        let index_dir = temp_dir.path().join(".cs");

        // Test normal file
        let sidecar = index_dir.join("test.txt.cs");
        let original = sidecar_to_original_path(&sidecar, &index_dir, temp_dir.path());
        assert_eq!(original, Some(PathBuf::from("test.txt")));

        // Test nested file
        let nested_sidecar = index_dir.join("src").join("main.rs.cs");
        let nested_original =
...
mod cleanup_validation {
    ...
    fn remove_manifest_entry(
        ...
        if cs_core::pdf::is_pdf_file(&standard_path) {
            let absolute_path = repo_root.join(&standard_path);
            let cache_path = cs_core::pdf::get_content_cache_path(repo_root, &absolute_path);
            if cache_path.exists() {
    ...
    fn cleanup_orphaned_sidecars(
        ...
        for entry in WalkDir::new(index_dir) {
            ...
            if entry.file_type().is_file() {
                ...
                if sidecar_path.extension().and_then(|s| s.to_str()) == Some("cs")
...
mod path_utils {
    ...
    pub fn get_sidecar_path_for_standard_path(index_dir: &Path, standard_path: &Path) -> PathBuf {
        let sidecar_name = format!("{}.cs", standard_path.display());
