...
/// Get default .ccignore file content
pub fn get_default_ccignore_content() -> &'static str {
    r#"# .ccignore - Default patterns for cc semantic search
...
/// Read and parse .ccignore file, returning patterns
pub fn read_ccignore_patterns(repo_root: &Path) -> Result<Vec<String>> {
    let ccignore_path = repo_root.join(".ccignore");

    if !ccignore_path.exists() {
        return Ok(Vec::new());
    }

    let content = std::fs::read_to_string(&ccignore_path).map_err(CcError::Io)?;
...
/// Create .ccignore file with default content if it doesn't exist
pub fn create_ccignore_if_missing(repo_root: &Path) -> Result<bool> {
    let ccignore_path = repo_root.join(".ccignore");
    ...
    std::fs::write(&ccignore_path, get_default_ccignore_content()).map_err(CcError::Io)?;
...
pub fn build_exclude_patterns(
    ...
    // 1. Load .ccignore patterns (highest priority among additional patterns)
    if use_ccignore
        && let Some(root) = repo_root
        && let Ok(ccignore_patterns) = read_ccignore_patterns(root)
        && !ccignore_patterns.is_empty()
    {
...
pub fn get_sidecar_path(repo_root: &Path, file_path: &Path) -> PathBuf {
    ...
    let mut sidecar = repo_root.join(".cc");
    sidecar.push(relative);
    let ext = relative
        .extension()
        .map(|e| format!("{}.cc", e.to_string_lossy()))
...
pub mod pdf {
    ...
    pub fn get_content_cache_path(repo_root: &Path, file_path: &Path) -> PathBuf {
        let relative = file_path.strip_prefix(repo_root).unwrap_or(file_path);
        let mut cache_path = repo_root.join(".cc").join("content");
    ...
    mod tests {
        ...
        fn test_get_content_cache_path() {
            ...
            assert_eq!(
                cache_path,
                PathBuf::from("/project/.cc/content/docs/manual.pdf.txt")
            );
        }
        ...
        fn test_get_content_cache_path_no_extension() {
            ...
            assert_eq!(
                cache_path,
                PathBuf::from("/project/.cc/content/docs/manual.txt")
            );
        }
...
mod tests {
    ...
    fn test_get_sidecar_path() {
        ...
        let expected = PathBuf::from("/home/user/project/.cc/src/main.rs.cc");
    ...
    fn test_get_sidecar_path_no_extension() {
        ...
        let sidecar = get_sidecar_path(&repo_root, &file_path);
        let expected = PathBuf::from("/project/.cc/README.cc");
    ...
    #[test]
    fn test_create_ccignore_if_missing() {
        let temp_dir = TempDir::new().unwrap();
        let test_path = temp_dir.path();

        // First creation should succeed
        let created = create_ccignore_if_missing(test_path).unwrap();
        assert!(created);

        // Check that file exists
        let ccignore_path = test_path.join(".ccignore");
        assert!(ccignore_path.exists());

        // Check content contains expected patterns
        let content = fs::read_to_string(&ccignore_path).unwrap();
        ...
        let created_again = create_ccignore_if_missing(test_path).unwrap();
        assert!(!created_again);
    }

    #[test]
    fn test_read_ccignore_patterns() {
        let temp_dir = TempDir::new().unwrap();
        let test_path = temp_dir.path();

        // Test with no .ccignore file
        let patterns = read_ccignore_patterns(test_path).unwrap();
        assert_eq!(patterns.len(), 0);

        // Create a .ccignore file
        let ccignore_path = test_path.join(".ccignore");
        fs::write(
            &ccignore_path,
            r#"# Comment line
...
"#,
    ...
    #[test]
    fn test_read_ccignore_patterns_with_empty_lines() {
        let temp_dir = TempDir::new().unwrap();
        let test_path = temp_dir.path();
...
"#,
    ...
    #[test]
    fn test_get_default_ccignore_content() {
        let content = get_default_ccignore_content();
